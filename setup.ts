...
import { AccountStatus, Address, Cell, Slice, Transaction } from "@ton/core";
  ...
  if (Array.isArray(subject)) {
    return {
      ...
      posMessage: ((subj: any[], cmp: FlatTransactionComparable) =>
        `Expected ${inspect(
          subj.map((tx) => flattenTransaction(tx))
        )} to contain a transaction that matches pattern ${inspect(cmp)}`).bind(
        undefined,
        subject,
        cmp
      ),
      negMessage: ((subj: any[], cmp: FlatTransactionComparable) =>
        `Expected ${inspect(
          subj.map((tx) => flattenTransaction(tx))
        )} NOT to contain a transaction that matches pattern ${inspect(
          cmp
    ...
    try {
      ...
      return {
        ...
        negMessage: ((flat: any, cmp: FlatTransactionComparable) =>
          `Expected ${inspect(flat)} NOT to match pattern ${inspect(
            cmp
          )}, but it does`).bind(undefined, flat, cmp),
      };
    } catch (e) {
...
function findTransaction<T extends Transaction>(
  txs: T | T[],
...
function findTransactionRequired<T extends Transaction>(
  txs: T | T[],
  match: FlatTransactionComparable
) {
  ...
  if (res === undefined) {
    throw new Error(
      `Expected ${inspect(
        Array.isArray(txs)
          ? txs.map((x) => flattenTransaction(x))
          : flattenTransaction(txs)
...
function filterTransactions<T extends Transaction>(
  txs: T[],
  match: FlatTransactionComparable
) {
...
function compareCellForTest(subject: any, cmp: Cell): CompareResult {
  return {
...
function compareAddressForTest(subject: any, cmp: Address): CompareResult {
  return {
    pass: cmp.equals(subject),
    posMessage: ((subject: any, cmp: Address) =>
      `Expected ${subject} to equal ${cmp}`).bind(undefined, subject, cmp),
...
function compareSliceForTest(subject: any, cmp: Slice): CompareResult {
  return {
